...
\subsection{Testcard to implementing test}
\label{sec:test-traceability-map}

Every entry below was recovered from the test sources themselves, not from the
specification: each name is the literal \texttt{BOOST\_AUTO\_TEST\_CASE}
identifier, or a script and its CTest registration. Files are abbreviated
\tBlock{RC} for \texttt{tests/testRollingContact.cpp}, \tBlock{RCR} for
\texttt{tests/testRollingContactRobot.cpp}, \tBlock{RCS} for
\texttt{tests/testRollingContactSolver.cpp} and \tBlock{LC} for
\texttt{src/mc\_control/samples/RollingContact/test\_controller\_lifecycle.cpp}.
A \emph{partial} entry covers part of the card's invariant under a different
name; \secRef{sec:test-gaps} says what is missing. Cards with no entry are
listed there too.

{\small
\begin{longtable}{|l|l|p{0.52\columnwidth}|}
  \hline
  Card & File & Implementing test \\
  \hline
  \endhead
  GEO-01 & RCR & \texttt{PlanarWheelOffsetsAreChassisHeadingInvariant} \\
  GEO-02 & RCR & \texttt{ChassisAlignedPlanarBasisIsOrthonormalRightHandedAndBodyConstant} \\
  GEO-03 & RCR & \texttt{DegenerateForwardAxisAbortsThePlanarBasis} \\
  GEO-04 & RC & \emph{partial} \texttt{RollingFramePointRowsSlipAndBias} (triad only) \\
  GEO-05 & RC & \emph{partial} \texttt{PlanarSpecializationsRecoverCommonChassisTwists} \\
  GEO-07 & RCR & \texttt{SteeringAngleIsChassisRelativeAndItsRateIsTheJointRateGEO07} \\
  GEO-08 & RCS & \emph{partial} \texttt{ReducedPlanarQPMatchesItsWholeBodyPreimageORC02} (annihilation),
    \texttt{LateralRowsAreStructurallyOverDeterminedOnThreePlanarDof} (sparsity) \\
  GEO-09 & RCS & \texttt{IcrConcurrencyIsExactlyTheLateralRankCondition} \\
  ROW-01 & RC & \texttt{DifferentialRowsAreTheExactDerivativeAtZeroGainROW01} \\
  ROW-02 & RC, RCS & \emph{partial} \texttt{PlanarSpecializationsRecoverCommonChassisTwists},
    \texttt{DifferentialDriveClosedFormHoldsOnTheSolvedPredictionORC03} \\
  ROW-03 & RCS & \texttt{LateralResidualDecaysAtTheDiscreteRateROW03} \\
  ROW-04 & RC & \texttt{PlanarRowsPinTheChassisAlignedBasisConventionROW04} \\
  ROW-05 & RC & \texttt{SteeringRowsAreTheExactDerivativeWithNoHdotTermROW05} \\
  ROW-06 & RC & \emph{partial} \texttt{RichardsonConvergenceOrderPinsTheHandDerivedDerivativesORC01},
    \texttt{SteeringRowsIncludeDirectionDerivative} \\
  ROW-07 & RCS & \texttt{RatePredictionIsAnExactAffineSubstitutionROW07} \\
  ROW-08 & RCS, RC & \texttt{LateralRowsAreStructurallyOverDeterminedOnThreePlanarDof},
    \texttt{DifferentialDriveRowsHaveNoDuplicateLateralConstraint} \\
  ROW-09 & RCS & \texttt{LateralSlacksAreTheRangeSpaceDefect} \\
  ROW-11 & RCS & \emph{adapted} \texttt{PredictedRateRowsCarryNoProportionalStabilizationROW11} \\
  DYN-01 & RCS & \texttt{ReducedInertiaIsACongruenceNotARowSliceDYN01} \\
  DYN-02 & RCS & \emph{adapted} \texttt{ReducedBiasCarriesTheVelocityProductTermDYN02} \\
  DYN-03 & RCS & \texttt{ContactForcesFollowNewtonsLawGeneralAndStaticDYN03} \\
  DYN-04 & RCS & \texttt{YawMomentumEqualsTheContactMomentDYN04} \\
  DYN-05 & RCS & \texttt{SteeringTorqueCannotEnterTheChassisYawRowDYN05} \\
  DYN-06 & RCS & \emph{adapted} \texttt{ContactPowerVanishesUnderIdealRollingDYN06} \\
  DYN-08 & RC & \emph{partial} \texttt{SteeringReferenceInvertsTheExpandedConstraints} (rim-speed sign) \\
  DYN-09 & RCS & \texttt{QuasiStaticTorqueBoundDuplicatesTheWheelRowDYN09} \\
  FRI-01 & RCS & \texttt{RollingContactDynamicsConstraintGeneratorRegularizationFRI01BiasAtEnabledAndTenX} \\
  FRI-02 & RCS & \texttt{FrictionConeIsRebuiltWhenTheWheelSteersFRI02} \\
  FRI-03 & RCS & \texttt{FrictionConeIsRebuiltWhenTheTerrainNormalChangesFRI03} \\
  FRI-04 & RCS & \emph{partial} \texttt{RollingDynamicConeFrameAndStableLayout} (upper bound only) \\
  FRI-08 & RCS & \emph{via} \texttt{SlopeSweepToFrictionAndTorqueFailureSMK09} (the band, both headings) \\
  BND-01 & RCS & \emph{partial} \texttt{YawingChassisConsumesNoSteeringRateBudgetBND02} \\
  BND-02 & RCS & \texttt{YawingChassisConsumesNoSteeringRateBudgetBND02} \\
  BND-06 & RCS & \texttt{LateralSlacksCarryNoBoxConstraint} \\
  BND-08 & RCS & \emph{partial} \texttt{RollingTasksModesKeepVariablesAndEnforceForcePolicy} \\
  QP-01 & RCS & \texttt{RollingContactDynamicsConstraintGeneratorRegularizationQP01EnabledClearsTheLibraryFloor} \\
  QP-02 & RCS & \texttt{RollingContactDynamicsConstraintGeneratorRegularizationQP02ZeroReliesOnlyOnTheLibraryFloor} \\
  QP-03 & RCS & \texttt{PrimalSolutionIsUniqueAcrossWarmStartsAndSolversQP03} \\
  QP-05 & RCS & \emph{partial} \texttt{HardLateralRowsFreezeTheChassisAndSoftOnesDoNot},
    \texttt{IncompatibleFourWheelDataSurfacesAsLateralSlack} \\
  QP-06 & LC & \emph{partial} \texttt{TwistWeightAxesAreUnitNormalizedNotInterchangeable} \\
  QP-07 & RCS & \texttt{KKTResidualIsAnIndependentOptimalityOracleQP07} \\
  QP-08 & RCS & \texttt{StabilityWindowOfTheProportionalStabilizationQP08} \\
  QP-09 & --- & \emph{partial} \texttt{rolling-contact-report/scripts/check-controller-log.py} (P99 budget) \\
  QP-10 & RCS & \texttt{RejectionContractNamesTheOffendingKeyQP10} \\
  ORC-01 & RC & \texttt{RichardsonConvergenceOrderPinsTheHandDerivedDerivativesORC01} \\
  ORC-02 & RCS & \texttt{ReducedPlanarQPMatchesItsWholeBodyPreimageORC02} \\
  ORC-03 & RCS & \texttt{DifferentialDriveClosedFormHoldsOnTheSolvedPredictionORC03} \\
  ORC-04 & RCS & \texttt{KnownIcrAnalyticOracleForFourSteeringORC04} \\
  ORC-05 & RC, RCS & \texttt{PlanarRowsSatisfyTheMetamorphicRelationsORC05},
    \texttt{WholeBodySolutionIsChassisHeadingInvariantORC05} \\
  ORC-06 & RCS & \texttt{SeededRandomizedPropertySweepORC06} \\
  ORC-08 & --- & \emph{adapted} \texttt{scripts/cross-check-mc-mujoco.py}, CTest
    \texttt{RollingContactMuJoCoCrossCheckORC08} and
    \texttt{RollingContactMuJoCoCrossCheckPDORC08}
    (\texttt{-DROLLING\_CONTACT\_MC\_MUJOCO\_CROSS\_CHECK=ON}) \\
  SMK-01 & RCS & \texttt{OneCycleAssemblyContractSMK01} \\
  SMK-02 & RCS & \emph{adapted} \texttt{StaticHoldOnFlatGroundAndOnARampSMK02} \\
  SMK-03 & LC & \emph{adapted} \texttt{StraightLineDriveTracksTheReferenceSMK03} \\
  SMK-04 & LC & \emph{adapted} \texttt{InPlaceRotationSMK04} \\
  SMK-05 & RCS & \texttt{ConstantRadiusArcCarriesTheCentripetalForceSMK05} \\
  SMK-06 & RCS & \texttt{CrabTranslationSeparatesT1FromT2SMK06} \\
  SMK-07 & RCS & \emph{adapted} \texttt{IncompatibleFourWheelDataSurfacesAsLateralSlack} \\
  SMK-08 & LC & \emph{adapted} \texttt{SteeringRateLimitedSlalomSMK08} \\
  SMK-09 & RCS & \texttt{SlopeSweepToFrictionAndTorqueFailureSMK09} \\
  SMK-10 & LC & \texttt{TangentBasisRotatesWithTheChassisSMK10} \\
  SMK-11 & LC & \emph{adapted} \texttt{OdometryCrossCheckIncludingWhereItMustDisagreeSMK11} \\
  SMK-12 & RCS & \texttt{TerrainTransitionKeepsGeometryAndConeContinuousSMK12} \\
  SMK-13 & LC & \emph{adapted} \texttt{LongHorizonClosedLoopSMK13},
    CTest \texttt{RollingContactLongHorizonSMK13}
    (\texttt{-DROLLING\_CONTACT\_LONG\_HORIZON\_SMOKE=ON}) \\
  \hline
\end{longtable}
}
...
\subsection{Cards adapted rather than implemented as written}
\label{sec:test-gaps-adapted}

Each entry below is a card whose stated invariant does not transfer to the
implementation under test, together with what is asserted in its place. In every
case asserting the card verbatim would assert something untrue, so the
divergence is recorded here rather than absorbed into a tolerance.

\begin{itemize}
  \item \tBlock{DYN-07, FRI-05, FRI-06, BND-07 --- the normal-load family has
    no target.} All four exercise \eqref{eq:planar-normal-load-row}, which is
    not implemented, and the decision not to implement it was measured rather
    than assumed. \secRef{sec:planar-normal-load} derives the exogenous
    normal-load row from $\bs{n}_{\Pi}^{\top}\text{J}_{\text{red},i}=0$, a
    property of the planar reduction. mc\_rtc solves the whole-body QP, where
    the floating base carries a vertical degree of freedom. Two measurements
    settled it: the ray $\generator{\force}\mapsto\generator{\force}+s\bs{1}$ is
    killed exactly and linearly by one non-optional equation-of-motion row --
    the base's vertical row, violated by $26.2$, $262$ and $2622$ at
    $s=1,10,100$ -- and appending the four normal kinematic rows raises the hard
    block's rank by $+3$ in both an ICR-coordinated and a non-coordinated
    steering configuration, so they are not $0=0$. The cone-shape premise
    $\text{C}_i\bs{1}\propto\bs{n}_i$ itself holds bit-for-bit. What the four
    cards were for is asserted elsewhere: the three out-of-plane equations are
    closed by the equations of motion and checked directly on the solved contact
    forces by DYN-03 and by SMK-02 at four slopes, and the cone binding of
    FRI-05 is measured by SMK-09.
  \item \tBlock{ROW-11 and the second half of SMK-03.} The card asserts that T1
    rows carry $-\pGain$ times the measured residual and T2 rows do not.
    \eqref{eq:four-steering-wheel-qp} is an ideal QP; the general
    acceleration-level rows mc\_rtc assembles carry $-\pGain\text{G}\bs{u}$ for
    \emph{both} chassis, so the asymmetry is neither true nor desirable here and
    asserting it would pin a defect. The test pins the genuinely T2-specific
    property instead -- that the predicted-rate rows carry no proportional term
    -- and SMK-03 asserts of both chassis what is true of both: the reference is
    tracked and the lateral residual is driven to zero rather than merely
    bounded.
  \item \tBlock{QP-10's non-finite-bound sub-item, declined.} The only path in
    this codebase that produces an infinite bound is the documented
    \texttt{infTorque} option, an intentional escape hatch rather than a defect,
    so rejecting it would break a supported configuration. The rest of the
    rejection contract is asserted.
  \item \tBlock{DYN-02's $\dot{\text{P}}\bs{u}$ term does not transfer.}
    RBDyn's floating-base $\bs{\alpha}$ is already the body twist of
    \eqref{eq:base-body-velocity} and the chassis-aligned basis is constant in
    that frame, so $\text{P}$ is constant along any planar motion and
    $\dot{\text{P}}=0$ \emph{exactly}. That is measured, not assumed: central
    differences return $0$ at two step sizes along the planar motion and $0.80$
    on the same probe once the chassis leaves the plane. The card's content
    transfers as ``the bias must be the full $\bs{h}(\bs{q},\bs{\alpha})$, not
    gravity alone'', which is asserted at $14.62$ against a
    $10^{-4}\mass\gravity[value]=0.0228$ tolerance, i.e. $642\times$ it.
  \item \tBlock{DYN-01's row-selection contrast does not bite on flat ground.}
    In mc\_rtc's coordinates $\text{P}$ degenerates to a selection when the
    contact plane is chassis-aligned, so the congruence and the principal
    submatrix coincide there and the gap is exactly $0$. The assertion with
    teeth is the tilted-plane branch, where the gap is $0.458$ and the selection
    also gets the kinetic energy wrong. Both branches are kept so the flat case
    is pinned as a property of these coordinates rather than mistaken for
    evidence.
  \item \tBlock{ORC-08's order-of-convergence pass condition is unachievable.}
    The fixtures run with \texttt{closedLoopFeedback: false}, so the control
    robot integrates open loop and its trajectory does not converge to the
    simulator's as $\samplingPeriod\to0$; the discrepancy is bounded by
    contact-model differences, by the simulator's own joint tracking and by real
    slip, none of which vanish with $\samplingPeriod$. The discrepancy is
    therefore measured at a ladder of timesteps and reported, and what is
    asserted are the trajectory-level invariants the ground-truth-injecting
    runner cannot observe.
  \item \tBlock{DYN-06's point-contact identity holds only for a non-yawing
    chassis.} mc\_rtc models each wheel as a two-point contact \emph{line} of
    width $w$, not the single point of \eqref{eq:ideal-rolling-velocity}, so a
    yawing chassis gives the endpoints a material velocity
    $\pm(w/2)\omega_{\Pi}$ that the point model does not have. The exact
    vanishing is asserted on a straight-line rollout and the finite-width term
    is reported next to it. The card's $5$\unitTime\ energy-balance rollout is
    not implemented: an Euler integration at $\samplingPeriod=5$\unitMs\ does
    not close the energy budget to $10^{-4}$, so the assertion would measure the
    integrator.
  \item \tBlock{QP-07's dual feasibility and complementary slackness are
    vacuous in the chosen fixture.} It carries no dynamics constraint, hence no
    contact multipliers and no cone, and no \texttt{KinematicsConstraint},
    hence no bounds. The test asserts that vacuity
    (\texttt{totalLambda() == 0}) rather than quietly reporting two conditions
    with nothing to satisfy; stationarity and primal feasibility are asserted,
    with a perturbation branch showing the oracle discriminates.
  \item \tBlock{SMK-02's twist bound is stated at $10^{-5}$\unitVelTS.} With
    contact forces of order $\mass\gravity[value]=228$\unitForce\ on a
    $23$\unitMass\ chassis, the QP's own numerical tolerance leaves about
    $10^{-3}$\unitAccTS\ of residual acceleration on a slope, i.e. twice the
    card's $10^{-6}$\unitVelTS. The bound is still $2500\times$ below the
    $2.5\times10^{-2}$\unitVelTS\ a chassis sliding freely at $30$\unitDegree\
    gains in one cycle, and the measured value is recorded at every slope.
  \item \tBlock{SMK-07's ``within $1\%$ of the predicted lower bound'' holds
    only asymptotically.} At the production lateral weight the QP trades the
    lateral rows against everything else and the realised slack sits well above
    the range-space defect. SMK-07 asserts the bound itself
    ($\norm{\slackVariable}\ge\norm{(\text{I}-\text{P})\bs{c}}$) and records
    both numbers; ROW-09 asserts the first-order convergence to it in
    $1/\weight_{\sigma}$, which is where the $1\%$ figure lives.
  \item \tBlock{SMK-09's torque half is inert on this vehicle.} The drive
    torque peaks at $26\%$ of its limit at the steepest slope in the sweep, so
    the failure the band predicts is a friction failure throughout. The torque
    margin is recorded at every slope so that a model with a weaker actuator
    would show the other branch.
  \item \tBlock{SMK-11's ramp bound is stated against the fall-line
    displacement.} The card's $\sin\theta$ times path length is the height lost
    by a straight run down the fall line. A differential chassis pointed down a
    $15$\unitDegree\ slope curves away from the commanded heading, because the
    instant it yaws at all gravity gains a body-lateral component while the hard
    lateral row still forbids lateral velocity at the carrier. Only the
    fall-line share of the path becomes height, and the bound uses the
    odometry's own estimate of it.
  \item \tBlock{SMK-13's strict non-monotonicity is gated on a noise floor.}
    T1's residual settles at $5\times10^{-10}$\unitVelTS, where a $22\%$ rise
    over the last $30$\unitTime\ is solver arithmetic; asserting
    non-monotonicity there would assert noise. The growth factor is asserted in
    both regimes. Its ramp branch is pinned on the chassis actually descending
    rather than on the residual, because tilting chassis and terrain together is
    a rigid rotation of the kinematic problem and the residual is legitimately
    identical on both terrains to eight digits.
  \item \tBlock{The companion Python harness is not part of this repository.}
    Fifteen cards say ``Implemented in \harness''. That file is not present
    here; twelve of them (ROW-01, ROW-03, ROW-04, ROW-05, ROW-08, DYN-03,
    DYN-04, DYN-05, FRI-01, QP-01, QP-02, QP-08) are implemented in the C++
    suite instead and are listed in
    \secRef{sec:test-traceability-map}; ROW-06 and FRI-04 are partial and
    FRI-05 is not implemented at all.
\end{itemize}

\subsection{Cards with no implementation}
\label{sec:test-gaps-unimplemented}

{\small
\begin{longtable}{|l|l|p{0.60\columnwidth}|}
  \hline
  Card & Prio. & Why not \\
  \hline
  \endhead
  GEO-06 & P1 & The contact-point offset is not a free parameter here:
    \texttt{contactPoint = carrierCenter - radius * n} is derived, so there is
    no independent quantity whose invisibility to the rows could be varied. \\
  GEO-10 & P1 & Single-source construction is structural in
    \texttt{RollingContactGeometry} -- one object produces the rows and the
    Jacobian -- so the card guards a duplication this implementation cannot
    have. \\
  ROW-10 & P1 & Degenerate steering configurations are covered indirectly by
    the ICR/rank equivalence of GEO-09 and by ROW-08's singular values; the
    explicit null-space direction is not asserted. \\
  ROW-12 & P2 & The $\pm\pi$ branch is handled inside
    \texttt{steeringWheelReference} and pinned behaviourally by
    \texttt{SteeringReferenceStaysWithinLimitsAndAvoidsBranchChatter}; the
    $2\pi$-periodicity of the rows themselves is not asserted. \\
  DYN-10 & P2 & A momentum-integral rollout needs a golden trace, which
    ORC-07 also wants and which is deliberately not stored (see below). \\
  FRI-09 & P2 & The circular and polyhedral margins are both computable, but
    only the polyhedral one is logged, so the ordering has nothing to compare
    against without adding a second diagnostic. \\
  FRI-10 & P1 & Its converse -- the sliding row is present and binding in
    \texttt{Sliding} -- is asserted by
    \texttt{RollingTasksModesKeepVariablesAndEnforceForcePolicy}; the negative
    direction is not. \\
  BND-03 & P1 & The rate box and the acceleration box do not coexist in this
    fixture: no \texttt{KinematicsConstraint} is attached, so there is no rate
    box to make jointly empty (see BND-06). \\
  BND-04 & P1 & Same cause: only the torque bound is present, so the ordering
    of thresholds the card wants has one member. \\
  BND-05 & P1 & The per-wheel torque stride is internal to
    \texttt{tasks::qp::MotionConstr}, which this suite treats as a dependency
    rather than as code under test. \\
  QP-04 & P0 & Adjacent coverage only: the exact row counts and labels are
    asserted by \texttt{RollingTasksMatrixPlacementAndHardCases} and by SMK-01,
    and the rank by ROW-08, but no single entry keeps the ledger the card
    asks for. \\
  ORC-07 & P2 & Deliberate. The card gates itself on QP-03 uniqueness and on
    strict complementarity; the $\generator{\force}$ block is not uniquely
    determined here (four coplanar contacts leave a one-dimensional null space
    in the normal-force split), so a golden file over it would fail on solver
    upgrades for no physical reason. \\
  \hline
\end{longtable}
}

\subsection{Open items}
\label{sec:test-gaps-open}

These are not omissions in the suite. They are behaviours the suite measured and
recorded, and which need a formulation or model change rather than another test.

\begin{itemize}
  \item \tBlock{Pure-yaw scrub torque.} A commanded in-place yaw is tracked at
    $6.7\%$ of the command against real \texttt{mc\_mujoco} under joint PD, and
    at $31\%$ under torque control, while every mixed command tracks at
    $84$--$96\%$. The reference itself is correct and mirror-symmetric; the
    hinges simply do not reach it. Narrowing the simulator's wheel collision
    half-width from $0.04$ to $0.004$\unitMeter\ -- ten times less contact line,
    everything else identical -- raises tracking from $31\%$ to $89\%$. The QP
    places its contact \emph{forces} along the wheel's full $0.08$\unitMeter\
    line but writes its rolling, lateral and normal \emph{constraints} at the
    single centre point, so it carries no term for the scrub of re-steering a
    loaded patch. A fix means constraining wheel spin about the contact normal,
    which is a formulation change. SMK-04 deliberately does not threshold yaw
    tracking, because a threshold the headless QP passes at $99.9\%$ would hide
    this rather than catch it.
  \item \tBlock{Torque-control tracking disagreement.} ORC-08 found that real
    \texttt{mc\_mujoco} delivers $67.9\%$ of the commanded crab distance under
    \texttt{--torque-control} against $98.6\%$ under joint PD, and that the gap
    does not converge with the timestep -- $0.2953$, $0.2151$ and
    $0.2914$\unitMeter\ at $5$, $2.5$ and $1$\unitMs. By the card's own
    criterion that is a controller/simulator disagreement rather than a
    discretization difference. No wheel detached and drive torque peaked at
    $6.5$~N$\cdot$m\xspace\ against a $35$~N$\cdot$m\xspace\ limit in either regime.
  \item \tBlock{Two failing MuJoCo cases.} \texttt{four-steering-rate} and
    \texttt{four-low-friction} fail in the CPU suite. Both are pre-existing and
    both are regressions against the archived evidence.
  \item \tBlock{Steering-rate ceiling at $4$\unitVelJS.} SMK-08 measured
    that the realised hinge rate follows the configured
    \texttt{maxSteeringRate} to within $3\%$ up to $4$\unitVelJS\ and is
    clamped at exactly $4.0$ above it, so the default -- the steering joints'
    own $8$\unitVelJS\ model velocity limit -- is unreachable and the QP
    delivers half the saturated rate the reference asks for. No bound in the
    fixture explains it: the steering torque at the plateau is
    $0.08$~N$\cdot$m\xspace\ against a $25$~N$\cdot$m\xspace\ limit, and the solved steering
    acceleration is exactly zero while the rate row still demands
    $-800$\unitAccVel. It is recorded by the test, not pinned as a contract.
\end{itemize}
